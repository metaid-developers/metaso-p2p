...
var defaultBackfillPaths = []string{pathFollow}
...
func (a *Aggregator) Backfill(opts BackfillOptions) error {
	if a == nil {
		return errors.New("social backfill aggregator is required")
	}
	client := opts.Client
	if client == nil {
		return errors.New("social backfill client is required")
	}
...
func normaliseBackfillPaths(paths []string) []string {
	...
	for _, path := range paths {
		normalised := normalisedPinPath(path)
