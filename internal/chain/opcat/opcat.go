...
// Chain implements chain.Chain for OPCAT.
// OPCAT is Bitcoin-derived and uses JSON-RPC (btcd rpcclient).
type Chain struct {
...
func (c *Chain) Name() string { return "opcat" }
...
func (c *Chain) Init() error {
	...
	if err != nil {
		return fmt.Errorf("opcat rpc connect: %w", err)
	}
	log.Println("[opcat] RPC connected")
	return nil
}
...
// NetParams returns the OPCAT network parameters.
