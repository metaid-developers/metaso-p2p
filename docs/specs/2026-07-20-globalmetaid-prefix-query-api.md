...
Status: proposed; API and index are not implemented yet.
...
creation-time-ordered secondary key per confirmed GlobalMetaID.
...
| `40400` | `globalMetaId not found` | The ready local confirmed index has no matching GlobalMetaID. |
...
“Earliest created” means the earliest confirmed root MetaID PIN (`path=/`) for
the canonical GlobalMetaID.
...
globalmetaid-prefix-state:v1
  -> {status, cursor, indexedCount, skippedCount, updatedAt}
```

`created-at-sort` and `height-sort` are fixed-width unsigned big-endian/hex
encodings, so byte order equals ascending numeric order. The value contains the
identity fields so the resolver does not need a second Pebble read or parse
variable key suffixes.
...
Add a dedicated command, tentatively
`cmd/metaso-p2p-globalmetaid-prefix-backfill`, which:

1. pages the configured MANAPI path-list source with `path=/`;
2. accepts confirmed root PINs with canonical valid GlobalMetaIDs and positive
   timestamps;
...
Before implementation, verify that the configured production MANAPI path-list
contract returns root PINs with `globalMetaId`, `timestamp`, `genesisHeight`,
`chainName`, and `id`. If any field is absent, the backfill must derive the
canonical GlobalMetaID from the root owner address using `pkg/idaddress`; it
must not invent a creation timestamp.
...
- `internal/storage`: only if a small reusable batch/early-stop iterator helper
  is preferable to using `OpenDB` directly;
