...
接口风格采用 metaso-p2p 主体接口约定：`code/message/data` 外层包裹，成功 `code=0`，业务错误使用非 0 code。v1 只定义 `40000/40400/50000` 三类错误码。列表使用 `data.list`，分页使用 opaque `nextCursor`。

## 部署决策

本需求的第一目标后端是 `metaso-p2p`，不另起独立 `bot-hub-server`。
...
- `metaso-p2p` 主程序必须 wire MVC indexer；`/protocols/skill-service` 当前主要发布在 MVC 链上。
...
- README / CLAUDE.md / GOAL_DRIVEN 等后端项目说明需要补充：`metaso-p2p` 同时承担 Bot Hub 聚合 API。
...
部署在 `metaso-p2p` 时，provider profile 解析必须走 in-process `userinfo` 聚合/PebbleDB 查询；HTTP 请求链路中不允许再通过 HTTP 调用自身或远端 manapi 来补 profile。
...
- 环境变量：`METASO_P2P_ASSET_BASE_URL`
- 推荐默认值：`https://manapi.metaid.io/content`
- 拼接规则：当资产字段是 pin id 或 content id 时，返回 `${METASO_P2P_ASSET_BASE_URL}/{id}`。
...
| Milestone | 内容 | 验收 |
| --- | --- | --- |
| M1 | `metaso-p2p` wire MVC indexer；索引 `/protocols/skill-service`；按 `originalId` 在同一 `chainName` 内折叠 create/modify/revoke；PebbleDB 持久化当前服务视图 | 构造同链 create+modify+revoke pin，验证聚合视图只保留当前有效服务，revoke 后默认不可见 |
...
| M4 | asset URL 解析；支持 `METASO_P2P_ASSET_BASE_URL` 配置 | serviceIcon/providerAvatar 都返回可加载 URL；配置缺省值可用 |
