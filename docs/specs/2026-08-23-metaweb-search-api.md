...
- **IDF-lite** (2026-08-23, per-namespace): per request, the document frequency of each scoring token is counted **per protocol key** over the merged snapshot (all sources, unfiltered; hit test over title+tags+summary+content). If `df[token][key]/docs[key] > 0.30`, that token's weight is multiplied by `0.5` when scoring docs of that protocol key — halved, not zeroed, so a direct query for a ubiquitous term (e.g. `metaid`) still works. Namespacing is by protocol key, not source aggregator, so a token common in `simplebuzz` does not affect `metabot-skill` scoring. In-memory only, two-pass scoring (one corpus scan for df, then the filtered scoring pass).
...
- **Protocol prior** (2026-08-23, IDBots P2): the final score (after field weights, IDF-lite, and the exact-phrase boost) is multiplied by a per-protocol-key prior so knowledge carriers win near-ties over chatter: `simplenote` ×1.2, `metaprotocol` ×1.2, `simplebuzz` ×0.9, `metaapp`/`metabot-skill`/`skill-service` ×1.0; unknown keys default to ×1.0. Applies to `sort=relevance` only (`sort=newest` keeps `score: 0`); the reported `score` is the prior-adjusted value. The prior never creates a match: multiplying a zero score stays zero.
